...
# ### FIX: Changed 'ResourceNotFound' to the correct 'ResourceDoesNotExist' ###
...
def get_or_create_canvas_user(details):
    ...
    try:
        ...
        # 4. Create user if all searches fail
...
def create_course(course_name):
    ...
    try:
        ...
        account = canvas.get_account(CANVAS_ACCOUNT_ID) # Re-fetch account if needed
...
def enroll_user_in_course(course_id, user_id, role):
    ...
    try:
        ...
        print(f"SUCCESS: Enrolled user {user_id} in course {course.name} as {role}.")
        return True
...
# ----- Functions below are for the legacy teacher webhook -----
...
def unenroll_teacher(course_id, teacher_details):
    ...
    # Teacher must exist to be unenrolled, so a simple lookup is sufficient.
    canvas = initialize_canvas_api()
    if not canvas: return False
    ...
    try:
        ...
        return unenroll_user_from_course(course_id, user.id)
         ...
         return True # Treat as success if user doesn't exist
    except Exception as e:
