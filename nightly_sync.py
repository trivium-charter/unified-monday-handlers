...
def process_plp_course_sync_webhook(event_data):
    ...
    for course_id in added_courses:
        ...
        # Check for ACE and process according to the new rules
        is_ace_course = course_secondary == "ACE"

        # Add to ACE column if it's an ACE course
        if is_ace_course:
            ace_col_id = PLP_CATEGORY_TO_CONNECT_COLUMN_MAP.get("ACE")
            if ace_col_id:
                course_to_final_cols[course_id].add(ace_col_id)
        
        # Determine other columns based on primary categories and ACE rules
        for category in tag_labels:
            if category in ["ELA", "Other/Elective"]:
                ...
                if primary_col_id:
                    course_to_final_cols[course_id].add(primary_col_id)
            elif not is_ace_course: # Only add other subjects if it's NOT an ACE course
                target_col_id = PLP_CATEGORY_TO_CONNECT_COLUMN_MAP.get(category)
                if target_col_id:
                    course_to_final_cols[course_id].add(target_col_id)
                else:
                    ...
                    if other_col_id:
                        ...
                        course_to_final_cols[course_id].add(other_col_id)
                    else:
