...
def manage_class_enrollment(action, plp_item_id, all_courses_item_id, student_details, user_id):
    ...
    if action == "enroll":
        ...
        if not canvas_api_id: return False
        canvas.enroll_user(canvas_api_id, student_details['canvas_user_id'], role)
    
    elif action == "unenroll":
        canvas_api_id = monday.get_canvas_api_id_from_all_courses_item(all_courses_item_id)
        if not canvas_api_id: return True # If no canvas course, nothing to unenroll from
...
def process_plp_course_sync_webhook(event_data):
    ...
    added_ids, removed_ids = curr_ids - prev_ids, prev_ids - curr_ids
    if not added_ids and not removed_ids: return True
    ...
    orig_val = monday.get_column_value(plp_item_id, int(PLP_BOARD_ID), target_plp_col_id).get('value')
    ...
    updated_val = monday.get_column_value(plp_item_id, int(PLP_BOARD_ID), target_plp_col_id).get('value')
    downstream_event = {'pulseId': plp_item_id, 'columnId': target_plp_col_id, 'value': updated_val, 'previousValue': orig_val}
...
def process_master_student_person_sync_webhook(event_data):
    ...
    for target_config in column_config.get("targets", []):
        linked_ids = monday.get_linked_items_from_board_relation(master_item_id, int(MASTER_STUDENT_BOARD_ID), target_config["connect_column_id"])
        for linked_item_id in linked_ids:
            monday.update_people_column(linked_item_id, target_config["board_id"], target_config["target_column_id"], current_value_raw, target_config["target_column_type"])
    return True
...
def process_sped_students_person_sync_webhook(event_data):
    ...
    linked_ids = monday.get_linked_items_from_board_relation(source_item_id, int(SPED_STUDENTS_BOARD_ID), SPED_TO_IEPAP_CONNECT_COLUMN_ID)
    for linked_id in linked_ids:
        monday.update_people_column(linked_id, int(IEP_AP_BOARD_ID), target_people_col_id, current_value_raw, target_col_type)
    return True
...
def process_teacher_enrollment_webhook(event_data):
    ...
    # THE FIX: This now uses the standardized variable name
